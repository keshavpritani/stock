...
$(document).ready(function() {
	...
	// top nav bar 
	$("#navPartyOrder").addClass('active');

	if(divRequest == 'add')  {
		// add order	
		// top nav child bar 
		$('#topNavAddPartyOrder').addClass('active');	
		...
		// top nav child bar 
		$('#topNavPartyManageOrder').addClass('active');
	...
	$.ajax({
		...
		success:function(response) {
			...
			for (var i = 0; i < response.length; i++) {
				var qty = $("#batch").val()*response[i][2];
				qty = qty.toFixed(3);
				var tr = 
				'<tr>'+
					'<td >'+response[i][1]+'</td>'+
					'<td >'+response[i][3]+'</td>'+
					'<td class = "qtyValue">'+response[i][2]+'</td>'+
					'<td>'+
						'<input type="number" id="qty'+i+'" class="form-control qty" value="'+qty+'" step="0.01" placeholder="Quantity" min="0" name="qty[]" autocomplete="off">'+
						'<input type="hidden" value="'+response[i][3]+'" name="stock[]">'+
						'<input type="hidden" value="'+response[i][0]+'" name="material[]">'+
					'</td>'+
				'</tr>';
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				...
				'<td style="padding-left:20px;">'+
					'<div class="form-group">'+
					'<input type="number" name="quantity[]" id="quantity'+count+'" autocomplete="off" class="form-control" min="1" style="width: 50%;float: right;"/>'+
					'</div>'+
				'</td>'+
				'<td>'+
					'<button class="btn btn-default removeProductRowBtn" type="button" onclick="removeProductRow('+count+')" style="float: right;"><i class="glyphicon glyphicon-trash"></i></button>'+
				'</td>'+
			'</tr>';
			if(tableLength > 0) {							
...
function subAmount() {
	...
	totalSubAmount = totalSubAmount.toFixed(3);
